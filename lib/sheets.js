...
const HEADER = ["日期時間", "星期", "類型", "分類", "項目", "金額", "UserId"];
...
async function appendExpense({ type, category, item, amount, userId, recordedAt }) {
  const sheets = await getSheetsClient();
  await ensureHeader(sheets);
  const timestamp = recordedAt || formatTaipeiTimestamp(new Date());
  ...
  await sheets.spreadsheets.values.append({
    ...
    requestBody: { values: [[timestamp, weekday, type, category, item, amount, userId]] },
  });
  return { timestamp };
}
